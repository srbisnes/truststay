...
export default function ListingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleBook() {
    ...
    setStatus("loading");
    setMessage("Preparing booking...");
    await new Promise((r) => setTimeout(r, 1800));
    setStatus("success");
    setMessage(`Booking simulated! ${nights} night(s) · Total ${total} USDC locked in escrow. After contract deployment this becomes a real on-chain tx.`);
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid lg:grid-cols-5 gap-10">
          ...
          <div className="lg:col-span-2">
            <div className="sticky top-24 p-6 rounded-2xl bg-white/5 border border-white/10 space-y-5">
              ...
              {nights > 0 && (
                <div className="space-y-2 text-sm border-t border-white/10 pt-4">
                  <div className="flex justify-between"><span className="text-slate-400">${listing.pricePerNight} × {nights} night{nights > 1 ? "s" : ""}</span><span>${listing.pricePerNight * nights}</span></div>
              ...
              <p className="text-xs text-slate-500 text-center">Funds stay in the smart contract until both parties confirm. Deposit returns automatically.</p>
            </div>
          </div>
        </div>
      </div>
